...
with patch("app.config.firebase.firebase_admin") as mock_firebase, \
    ...
    def test_session_login_missing_token(client):
        ...
        assert "idToken is required" in data["error"]

    def test_session_login_success(client):
        # We need to mock auth.verify_id_token and auth.create_session_cookie
        with patch("app.routes.auth.auth.verify_id_token") as verify_mock, \
             patch("app.routes.auth.auth.create_session_cookie") as create_mock:
             
            import datetime
            verify_mock.return_value = {"auth_time": datetime.datetime.now().timestamp() - 60}
            create_mock.return_value = "mock_session_cookie"
            
            res = client.post("/api/v1/auth/sessionLogin", json={"idToken": "fake_token"})
    ...
    def test_session_logout(client):
        client.set_cookie("localhost", "session", "fake_session")
        with patch("app.routes.auth.auth.verify_session_cookie") as verify_mock, \
